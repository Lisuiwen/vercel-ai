...
1. 用户在聊天 UI 中输入消息。
1. 消息发送到 API 路由。
1. 在服务端路由中，语言模型在 `streamText` 调用期间生成工具调用。
1. 所有工具调用转发到客户端。
1. 服务端工具使用其 `execute` 方法执行，结果转发到客户端。
1. 应自动执行的客户端工具通过 `onToolCall` 回调处理。你必须调用 `addToolOutput` 提供工具结果。
1. 需要用户交互的客户端工具可在 UI 中显示。工具调用和结果可作为工具调用部分，在最后一条助手消息的 `parts` 属性中访问。
1. 用户交互完成后，可使用 `addToolOutput` 将工具结果添加到聊天。
1. 可使用 `sendAutomaticallyWhen` 配置聊天，在所有工具结果可用时自动提交。这将触发此流程的另一次迭代。
