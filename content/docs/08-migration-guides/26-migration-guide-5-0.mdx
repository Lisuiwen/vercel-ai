---
title: 将 AI SDK 从 4.x 迁移到 5.0
description: 学习如何将 AI SDK 从 4.x 升级到 5.0。
---

# 将 AI SDK 从 4.x 迁移到 5.0

## 推荐迁移流程

1. 备份项目。若使用版本控制系统，请确保先前版本均已提交。
1. 升级到 AI SDK 5.0。
1. 使用以下方式之一自动迁移代码：
   - 在 Cursor 或其他支持 MCP 的编码 Agent 中使用 [AI SDK 5 Migration MCP Server](#ai-sdk-5-migration-mcp-server) 进行 AI 辅助迁移
   - 使用 [codemods](#codemods) 自动转换代码
1. 按下方破坏性变更指南调整。
1. 确认项目运行正常。
1. 提交变更。
...
[AI SDK 5 Migration Model Context Protocol (MCP) Server](https://github.com/vercel-labs/ai-sdk-5-migration-mcp-server) 可通过编码 Agent 自动迁移项目。该服务器面向 Cursor 设计，也适用于支持 MCP 的其他编码 Agent。

开始前，在项目中创建或编辑 `.cursor/mcp.json`：
