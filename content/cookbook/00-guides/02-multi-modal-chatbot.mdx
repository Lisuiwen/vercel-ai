---
title: 多模态 Agent
description: 学习如何使用 AI SDK 构建可处理图像与 PDF 的多模态 agent
tags: ['multi-modal', 'agent', 'images', 'pdf', 'vision', 'next']
---

# 多模态 Agent

本指南将构建能理解图像与 PDF 的多模态 agent。

多模态指 agent 能以多种格式理解并生成回复。本指南聚焦图像与 PDF——现代语言模型可原生处理的常见文档类型。

<Note>
  各提供商多模态能力完整列表请参阅[提供商文档](/providers/ai-sdk-providers)。
</Note>

示例使用 OpenAI 的 GPT-4o，但相同代码可无缝用于其他提供商——只需改一行即可切换。
