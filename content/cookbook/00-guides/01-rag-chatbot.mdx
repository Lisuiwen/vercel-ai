---
title: RAG Agent
description: 学习如何使用 AI SDK 与 Next.js 构建 RAG Agent
tags:
  [
    'rag',
    'chatbot',
...
深入之前，先了解什么是 RAG 以及为何使用它。

### 什么是 RAG？

RAG 即检索增强生成（Retrieval Augmented Generation）。简单说，RAG 是在提示时向大语言模型（LLM）提供与问题相关的特定信息。

### 为何需要 RAG？

LLM 虽强大，但推理范围限于训练数据。询问训练数据之外的信息（专有数据或训练截止后的常识）时问题尤为明显。RAG 通过获取与提示相关的信息并作为上下文传给模型来解决这一问题。

用一个简单例子说明：假设询问模型你最喜欢的食物：
...
这样即可通过为查询提供相关信息来增强模型生成。只要模型拥有合适信息，就很可能对用户问题给出准确回答。那如何检索相关信息？答案依赖称为嵌入（embedding）的概念。

<Note>
  RAG 应用可获取任意上下文（例如 Google 搜索）。嵌入与向量数据库只是实现语义搜索的一种检索方式。
</Note>

### 嵌入

[嵌入](/docs/ai-sdk-core/embeddings)将词、短语或图像表示为高维空间中的向量。空间中相近的词彼此接近，向量间距离可衡量相似度。

实践中，嵌入 `cat` 与 `dog` 时，它们在向量空间中应彼此靠近。计算两向量相似度的过程称为「余弦相似度」，值为 1 表示高度相似，-1 表示高度相反。

<Note>
  若觉得复杂不必担心，入门只需高层理解即可！更深入的嵌入介绍请参阅[本指南](https://jalammar.github.io/illustrated-word2vec/)。
</Note>
