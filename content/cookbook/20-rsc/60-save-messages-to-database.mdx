---
title: 将消息保存到数据库
description: 学习如何使用 AI SDK 与 React Server Components 将消息保存到外部数据库
tags: ['rsc', 'tool use']
---

# 将消息保存到数据库

与语言模型的对话有时值得保存，以便稍后回顾或继续。

`createAI` 提供实验性回调 `onSetAIState`，在 AI 状态变化时触发，可将 AI 状态保存到文件或数据库。
