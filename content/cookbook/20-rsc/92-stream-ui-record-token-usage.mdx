---
title: 流式 UI 后记录 Token 用量
description: 学习如何在流式用户界面后使用 AI SDK 与 React Server Components 记录 token 用量
tags: ['rsc', 'usage']
---

# 流式 UI 后记录 Token 用量

使用 [`streamUI`](/docs/reference/ai-sdk-rsc/stream-ui) 流式传输结构化数据时，可能需要记录 token 用量以便计费。

## `onFinish` 回调

可用 `onFinish` 回调记录 token 用量，在流结束时调用。
