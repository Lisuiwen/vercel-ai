---
title: 在组件间共享 useChat 状态
description: 学习如何在多个组件间共享 useChat 聊天实例并轻松重置对话
tags: ['next', 'useChat', 'context']
---

# 在组件间共享 useChat 状态

构建聊天应用时，你可能希望在多个组件中访问同一聊天实例：在一个组件展示消息，在另一个处理输入，并在应用任意位置控制聊天状态。

## 创建聊天 Context

首先创建一个 context，用于保存聊天实例并提供交互方法。
...
在 layout 中加入 `ChatProvider`，使所有子组件都能访问聊天 context。
